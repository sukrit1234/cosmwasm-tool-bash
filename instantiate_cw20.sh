...
echo "Input token decimal"
read TOKEN_DECIMAL
if ! [[ $TOKEN_DECIMAL =~ ^[0-9]+\.?[0-9]*$ ]]; then
   echo "Error : Token decimal must be unsigned integer"
   exit 1
fi

if [[ $TOKEN_DECIMAL -lt 0 ]] || [[ $TOKEN_DECIMAL -gt 18 ]]; then
   echo "Error : Decimal is overflow from range [0-18]"
   exit 1
fi 

echo "Input instant label if leave this 'CW20Token' will be used"
read INSTANCE_LABEL
if [[ -z $INSTANCE_LABEL ]]; then
   INSTANCE_LABEL="CW20Token"
...
if WALLET_ADDRESS=$(wasmd keys show -a $2); then
   ...
   INSTANT_ARGS+='","decimals":'
   INSTANT_ARGS+=$TOKEN_DECIMAL
   INSTANT_ARGS+=',"initial_balances":[]'
   INSTANT_ARGS+=',"mint":{"minter":"' 
   INSTANT_ARGS+=$WALLET_ADDRESS
   INSTANT_ARGS+='"}}'
